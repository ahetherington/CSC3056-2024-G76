...
public class SimpleBankingAppTest {
	...
	public static void testLoadData() {
		...
		if (SimpleBankingApp.accounts.size() == 4)
			System.out.println(TestUtils.TEXT_COLOR_GREEN + "testLoadData: loadAccountData: TC2 passed"
					+ TestUtils.TEXT_COLOR_RESET);
		else
			System.out.println(TestUtils.TEXT_COLOR_RED + "testLoadData: loadAccountData: TC2 FAILED"
					+ TestUtils.TEXT_COLOR_RESET);
	...
	public static void testDeposits() {
		...
		if (balanceBefore + depositAmount == balanceAfter)
			System.out.println(TestUtils.TEXT_COLOR_GREEN + "testDeposits: TC3 passed" + TestUtils.TEXT_COLOR_RESET);
		else {
			System.out.println(TestUtils.TEXT_COLOR_RED
					+ "testDeposits: TC3 FAILED XXX: balanceBefore + depositAmount != balanceAfter");
	...
	public static void testWithdrawals() {
		...
		// 2-Exercise phase
		SimpleBankingApp.addTransaction("5495-1234", -withdrawalAmount); // Assuming the method supports negative for
																			// withdrawals
		double balanceAfter = SimpleBankingApp.getBalance("5495-1234");
		...
		if (balanceBefore - withdrawalAmount == balanceAfter)
			System.out.println(TestUtils.TEXT_COLOR_GREEN + "testWithdrawals: TC4 passed" + TestUtils.TEXT_COLOR_RESET);
		else {
			System.out.println(TestUtils.TEXT_COLOR_RED
					+ "testWithdrawals: TC4 FAILED XXX: balanceBefore - withdrawalAmount != balanceAfter");
			System.out.format(
