...
public class SimpleBankingApp {
	...
	public static void printAllUsers() {
		...
		System.out.println(String.format("%-25s| %-15s| %-15s| %-15s| %-15s", "username", "password", "first_name",
				"last_name", "mobile_number"));
		System.out
				.println("-------------------------------------------------------------------------------------------");
	...
	public static void printAllAccounts() {
		...
		System.out.println(String.format("%-10s| %-30s| %-10s| %-15s| %-15s", "Account #", "username_of_account_holder",
				"type", "opening_date", "Balance"));
		System.out.println("--------------------------------------------------------------------------------");
	 ...
	 * needs to go over all transactions that match the account and get their sum
	 * total. For example, if an account has only two transactions in the system,
	 * with values = $10.79 and $-140, the balance would be $-129.21
	 * 
	 * @param account_number
	 * @return A double value, being the balance of the account
	 */
	public static double getBalance(String account_number) {
		double balance = 0.0; // Balance value initialised which will store the final balance for the selected
								// account

		// A for loop will iterate through each transaction object in the system
		for (Transaction transaction : transactions) {
			/*
			 * The if command will check if the account number of the current transaction
			 * matches the account number which has been passed to it
			 */
			if (transaction.getAccount_number() == (account_number)) {
				// If the transaction belongs to the account, the specified amount is
				// added/subtracted to the balance
				balance += transaction.getTransaction_amount();
			}
		}

		return balance; // Final balance returned after the for loop finishes iterating through all
						// transactions
	}
